...
<!-- 头部 -->
 ...
 <van-swipe class="my-swiper" :autoplay="3000" indicator-color="white" height="170px">
  <van-swipe-item v-for="item in homeData.slides" :key="item.id">
    <van-image :src="item.pic_image_url" radius="5px 5px"></van-image>
  </van-swipe-item>
</van-swipe>
<!-- 快捷入口 -->
 <van-row justify="space-around">
  <van-col span="12" v-for="(item,index) in homeData.nav2s" :key="item.id" class="center-img" @click="goOrderTwo(index)">
    <van-image :src="item.pic_image_url" radius="5px 5px"></van-image>
  </van-col>
</van-row>
<!-- 医院列表 -->
 <van-row justify="space-around" @click="goOrder(item)" v-for="(item,index) in homeData.hospitals" :key="item.id" class="yy-list">
     <van-col span="6">
       <van-image :src="item.avatar_url" radius="5px 5px" width="100" height="90"></van-image>
     </van-col>
...
const goOrderTwo=(index)=>{
    router.push(`/createOrder?${homeData.hospitals[index].id}`)
}
//医院点击
const goOrder=(data)=>{
    router.push(`/createOrder?${data.id}`)
}
</script>

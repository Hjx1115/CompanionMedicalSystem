<template>
  <div class="container">
    <div class="header">
      <van-icon name="arrow-left" @click="goBack" class="header-left" size="20px"/>
      订单详情
    </div>
    <status-bar :item="stateMap[detailInfo.trade_state]" />
    <div class="tips">
      <div class="dzf" v-if="detailInfo.trade_state === '待支付'">
      ...
      <div class="dzf" v-if="detailInfo.trade_state === '待服务'">
        <div class="text1">正在为您安排服务专员...</div>
        <div class="text2">请保持手机通畅，稍后将有服务专员与您联系</div>
      </div>
      <div class="dzf" v-if="detailInfo.trade_state === '已完成'">
        <div class="text1">服务已完成</div>
        <div class="text2">感谢您的使用，如有售后问题请联系客服</div>
      </div>
      <div class="dzf" v-if="detailInfo.trade_state === '已取消'">
...
const seconds=computed(()=>{
  if(detailInfo.order_start_time){
    return detailInfo.order_start_time + 7200000 - Date.now();
  }else return 0
})
//订单详情组装（预约信息）
const makeInfo={
...
const formatData = (key) => {
  if (key.indexOf(".") === -1) {
    if (key === "order_start_time") {
      return formatTimestamp(detailInfo[key]);
    }
